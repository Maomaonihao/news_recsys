...
class OnlineServer(object):
    ...
    def get_cold_start_rec_list_v2(self, user_id, age=None, gender=None):
        ...
        # 通过轮询的方式
        user_news_list, exposure_news_list = self._get_polling_rec_list(user_id, news_expose_set, cate_id_list,
                                                                        rec_type="cold_start")
    ...
    def get_hot_list_v2(self, user_id):
        ...
        # 通过轮询的方式获取用户的展示列表
        user_news_list, exposure_news_list = self._get_polling_rec_list(user_id, news_expose_set, self.cate_id_list,
                                                                        rec_type="hot_list")
    ...
    def test(self):
        user_info = self.register_sql_sess.query(RegisterUser).filter(
            RegisterUser.userid == "4566566568405766145").first()
        print(user_info.age)
