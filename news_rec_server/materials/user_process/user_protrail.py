...
class UserProtrail(object):
    ...
    def _user_info_to_dict(self, user):
        ...
        time_range = 15
        _, feature_dict = self.get_statistical_feature_from_history_behavior(user.userid, time_range,
                                                                             behavior_types=behaviors)
        for feature_type in feature_dict.keys():
            if feature_dict[feature_type]:
                info_dict["{}_{}_intr_cate".format(feature_type, time_range)] = \
                    feature_dict[feature_type]["intr_cate"]  # 历史喜欢最多的Top3的新闻类别
                info_dict["{}_{}_intr_key_words".format(feature_type, time_range)] = \
                    feature_dict[feature_type]["intr_key_words"]  # 历史喜欢新闻的Top3的关键词
                info_dict["{}_{}_avg_hot_value".format(feature_type, time_range)] = \
                    feature_dict[feature_type]["avg_hot_value"]  # 用户喜欢新闻的平均热度
                info_dict["{}_{}_news_num".format(feature_type, time_range)] = \
                    feature_dict[feature_type]["news_num"]  # 用户15天内喜欢的新闻数量
            else:
                info_dict["{}_{}_intr_cate".format(feature_type, time_range)] = ""  # 历史喜欢最多的Top3的新闻类别
                info_dict["{}_{}_intr_key_words".format(feature_type, time_range)] = ""  # 历史喜欢新闻的Top3的关键词
                info_dict["{}_{}_avg_hot_value".format(feature_type, time_range)] = 0  # 用户喜欢新闻的平均热度
                info_dict["{}_{}_news_num".format(feature_type, time_range)] = 0  # 用户15天内喜欢的新闻数量
    ...
    def get_statistical_feature_from_history_behavior(self, user_id, time_range, behavior_types):
        ...
        for type in behavior_types:
            ...
            try:
                history = sess.query(table_obj).filter(table_obj.userid == user_id).filter(
                    table_obj.curtime >= start).filter(table_obj.curtime <= end).all()
    ...
    def _gen_statistical_feature(self, history):
        ...
        feature_dict = dict()
        # 计算平均热度
        feature_dict["avg_hot_value"] = 0 if sum(history_hot_value) < 0.001 else sum(history_hot_value) / len(
            history_hot_value)
        ...
        cate_str = ",".join(
            [item[0] for item in cate_list[:3]] if len(cate_list) >= 3 else [item[0] for item in cate_list])
        ...
        word_str = ",".join(
            [item[0] for item in word_list[:3]] if len(cate_list) >= 3 else [item[0] for item in word_list])
...
if __name__ == "__main__":
    UserProtrail().update_user_protrail_from_register_table()
